...
export const CustomerPortal = () => {
  ...
  if (!customerId) return <div className="text-white text-center">Invalid Portal Link</div>;

  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center pb-4 border-b border-slate-700">
        <div>
          <h2 className="text-3xl font-bold text-white tracking-tight">Your Quotations</h2>
          <p className="text-slate-400 mt-2">Review, accept, or request changes to your pending deals.</p>
        </div>
      </div>

      {isLoading ? (
        <div className="text-slate-400">Loading your deals...</div>
      ) : quotations?.length === 0 ? (
        <div className="card text-center py-12">
          <FileText className="h-12 w-12 text-slate-500 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-white mb-2">No Active Quotations</h3>
          <p className="text-slate-400">You don't have any pending quotations to review at the moment.</p>
        </div>
      ) : (
            ...
            <div key={q.id} className="card p-6 border-l-4 border-l-primary flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-xl font-bold text-white">{q.quotation_number}</h3>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 text-sm">
                  <div>
                    ...
                    <p className="text-slate-300">${q.subtotal}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-slate-300">${q.tax_total}</p>
                  </div>
                  <div>
                    <p className="text-slate-500 mb-1">Grand Total</p>
                    <p className="text-white font-bold text-lg">${q.grand_total}</p>
                  </div>
                </div>

                <div className="mt-4 border-t border-slate-700 pt-4">
                  <p className="text-sm font-medium text-slate-300 mb-2">Items Included:</p>
              ...
              <div className="md:w-64 flex flex-col justify-center space-y-3 border-t md:border-t-0 md:border-l border-slate-700 pt-4 md:pt-0 md:pl-6">
                {q.status === 'APPROVED' ? (
                  <>
                    <button 
                ...
                {negotiateQuoteId === q.id && (
                  <div className="mt-4 p-4 bg-slate-800 rounded-lg animate-in fade-in zoom-in-95 duration-200">
                    <p className="text-sm text-slate-300 mb-2">What would you like to change?</p>
                    <textarea 
                      value={notes}
                      onChange={(e) => setNotes(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-700 rounded-md p-2 text-sm text-white focus:border-primary focus:ring-1 focus:ring-primary mb-3"
                      rows={3}
