  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">Fulfillment Center</h2>
          <p className="text-sm text-slate-400 mt-1">Manage orders and allocate inventory</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <h3 className="text-lg font-medium text-white mb-4">Pending Allocations (Approved Quotes)</h3>
          <div className="space-y-4">
            {pendingQuotations?.length === 0 ? (
              <p className="text-slate-400 text-sm">No approved quotations awaiting allocation.</p>
            ) : (
              pendingQuotations?.map((q: any) => (
                <div key={q.id} className="border border-slate-700 rounded-lg p-4 bg-slate-800/50 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-slate-400">Total: ${q.grand_total} | Items: {q.items?.length || 0}</p>
                  </div>
                  <button 
        ...
        <div className="card p-0">
          <div className="p-4 border-b border-slate-700 bg-slate-800/50 rounded-t-lg">
            <h3 className="text-lg font-medium text-white">Active Fulfillment Orders</h3>
          </div>
            ...
            <table className="min-w-full divide-y divide-slate-700">
              ...
              <tbody className="bg-surface divide-y divide-slate-700">
                {loadingOrders ? (
                  ...
                  <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400">No orders found.</td></tr>
                ) : (
                  orders?.map((order: any) => (
                    <tr key={order.id} className="hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-3 text-sm text-white">{order.quotation_id.substring(0, 8)}...</td>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-400">{order.tracking_number || 'Pending'}</td>
                    </tr>
                  ))
                )}
              </tbody>
