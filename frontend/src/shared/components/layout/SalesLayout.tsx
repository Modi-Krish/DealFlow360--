...
import { LayoutDashboard, FileText, CheckCircle, Package, LogOut } from 'lucide-react';
      ...
      <div className="w-64 bg-surface border-r border-slate-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto py-4">
          <div className="px-4 mb-4">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Sales Portal</p>
          </div>
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center mb-4">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-white">{user?.name || 'Sales Rep'}</p>
              <p className="text-xs text-slate-400">{user?.role || 'SALES_REP'}</p>
            </div>
          </div>
          <button 
            onClick={logout}
